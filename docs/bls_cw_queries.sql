-- ============================================================================
-- BLS CW (Consumer Price Index - Urban Wage Earners) SQL Query Documentation
-- ============================================================================
-- Survey: Consumer Price Index for Urban Wage Earners and Clerical Workers
-- Tables: bls_cw_areas, bls_cw_items, bls_cw_series, bls_cw_data, bls_cw_aspects
...
-- List all CW areas with hierarchy
SELECT
    area_code,
    area_name,
    display_level,
...
-- List all CW items (commodities/services)
SELECT
    item_code,
    item_name,
    display_level,
...
-- Find series for U.S. All Items (headline CPI-W)
SELECT
    s.series_id,
    s.series_title,
    a.area_name,
    ...
    s.end_year,
    s.is_active
FROM bls_cw_series s
JOIN bls_cw_areas a ON s.area_code = a.area_code
JOIN bls_cw_items i ON s.item_code = i.item_code
...
-- Get latest CPI-W value (U.S. All Items, Seasonally Adjusted)
SELECT
    d.year,
    d.period,
    d.value,
...
SELECT
    ...
    i.item_name
FROM bls_cw_data d
JOIN bls_cw_series s ON d.series_id = s.series_id
JOIN bls_cw_areas a ON s.area_code = a.area_code
JOIN bls_cw_items i ON s.item_code = i.item_code
WHERE d.year = 2024
  AND d.period = 'M09'
...
-- Compare CPI-W across major regions (latest month)
WITH latest_data AS (
    SELECT
        series_id,
        year,
...
FROM bls_cw_series s
JOIN bls_cw_data d ON s.series_id = d.series_id
...
FROM bls_cw_series s
LEFT JOIN bls_cw_data d ON s.series_id = d.series_id
WHERE s.is_active = true
...
FROM bls_cw_items i
JOIN bls_cw_series s ON i.item_code = s.item_code
LEFT JOIN bls_cw_data d ON s.series_id = d.series_id
WHERE s.is_active = true
