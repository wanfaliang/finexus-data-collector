"""
Collect Nasdaq Stock Screener Data
Downloads and processes daily stock screener data from Nasdaq
"""
import sys
import argparse
import logging
...
from src.collectors.nasdaq_screener_collector import NasdaqScreenerCollector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/nasdaq_screener_collect.log', encoding='utf-8'),
...
def main():
    parser = argparse.ArgumentParser(
        description='Collect Nasdaq stock screener data',
...
Examples:
  ...
  python scripts/collect_nasdaq_screener.py --csv-file data/nasdaq_screener/nasdaq_screener_20251111_210633.csv
  ...
  python scripts/collect_nasdaq_screener.py --force
        """
    )
    ...
    logger.info("="*80)
    logger.info(f"NASDAQ STOCK SCREENER DATA COLLECTION - {datetime.now()}")
    logger.info("="*80)
    ...
    try:
        with get_session() as session:
            collector = NasdaqScreenerCollector(session)
            ...
            if success:
                ...
                if latest_date:
                    ...
                    logger.info(f"Total symbols in latest snapshot: {symbol_count:,}")
        ...
        logger.error("  1. Manually download CSV from: https://www.nasdaq.com/market-activity/stocks/screener")
        logger.error("  2. Click 'Download CSV' button and save to data/nasdaq_screener/")
        logger.error("  3. Run: python scripts/collect_nasdaq_screener.py --csv-file <path-to-csv>")
        logger.error("  OR")
        logger.error("  Try: python scripts/download_nasdaq_screener.py --no-headless")
        return 1
