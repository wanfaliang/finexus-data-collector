...
Fetches earnings announcements calendar from FMP API
Supports upcoming earnings, specific date ranges, and historical backfill
"""
import sys
import argparse
import logging
...
from src.collectors.earnings_calendar_collector import EarningsCalendarCollector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/earnings_calendar.log'),
...
def main():
    parser = argparse.ArgumentParser(
        description='Collect earnings calendar data from FMP API',
...
Examples:
  ...
  python scripts/collect_earnings_calendar.py --from 2024-01-01 --to 2024-03-31

  # Backfill last year
  python scripts/collect_earnings_calendar.py --backfill-days 365
  ...
  python scripts/collect_earnings_calendar.py --backfill-from 2020-01-01
    ...
    logger.info("="*80)
    logger.info("EARNINGS CALENDAR COLLECTION")
    logger.info("="*80)
    ...
    with get_session() as session:
        ...
        if args.from_date and args.to_date:
            ...
            if result['success']:
                ...
                logger.info(f"  Announcements received: {result['announcements_received']:,}")
                logger.info(f"  Announcements upserted: {result['announcements_upserted']:,}")
                logger.info(f"{'='*80}")
                return 0
            else:
                logger.error(f"\n{'='*80}")
            ...
            logger.info(f"  Total announcements: {result['total_announcements']:,}")
            logger.info(f"{'='*80}")
            ...
            # Default: upcoming announcements
