#!/usr/bin/env python3
"""
Update CE data with latest observations from BLS API
...
Usage:
    ...
    python scripts/bls/update_ce_latest.py --start-year 2024
    python scripts/bls/update_ce_latest.py --limit 100
    python scripts/bls/update_ce_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Update CE data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW CE DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
    ...
    try:
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'ce',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                CEData.series_id,
                func.max(CEData.year).label('max_year')
            ).filter(
                CEData.series_id.in_(series_ids)
            ).group_by(
                CEData.series_id
            ).all()
