#!/usr/bin/env python3
"""
Update SU data with latest observations from BLS API

This script fetches the latest data points for SU series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_su_latest.py
    python scripts/bls/update_su_latest.py --start-year 2024
    python scripts/bls/update_su_latest.py --limit 100
    python scripts/bls/update_su_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
from database.bls_models import SUSeries, SUData
from database.bls_tracking_models import BLSSeriesUpdateStatus, BLSAPIUsageLog
from config import settings

def main():
    parser = argparse.ArgumentParser(description="Update SU data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    parser.add_argument(
        '--items',
        help='Item codes (comma-separated)'
    )
    parser.add_argument(
        '--seasonal',
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW SU DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING SU (Chained Consumer Price Index for All Urban Consumers) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if args.series_ids:
            ...
            query = session.query(SUSeries.series_id).filter(SUSeries.is_active == True)
            # Apply survey-specific filters
            if args.areas:
                filter_values = [v.strip() for v in args.areas.split(',')]
                query = query.filter(SUSeries.area_code.in_(filter_values))
                print(f"Filter: area_code in {filter_values}")
            if args.items:
                filter_values = [v.strip() for v in args.items.split(',')]
                query = query.filter(SUSeries.item_code.in_(filter_values))
                print(f"Filter: item_code in {filter_values}")
            if args.seasonal:
                filter_values = [v.strip() for v in args.seasonal.split(',')]
                query = query.filter(SUSeries.seasonal_code.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'su',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                SUData.series_id,
                func.max(SUData.year).label('max_year')
            ).filter(
            ...
            for batch_num, i in enumerate(range(0, len(series_ids), batch_size), 1):
                ...
                try:
                    ...
                    if data_to_upsert:
                        stmt = insert(SUData).values(data_to_upsert)
                    ...
                    for series_id in batch:
                        ...
                        latest = session.query(
                            func.max(SUData.year)
                        ).filter(
                            SUData.series_id == series_id
                        ).scalar()
