#!/usr/bin/env python3
"""
Update SM data with latest observations from BLS API

This script fetches the latest data points for SM series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_sm_latest.py
    python scripts/bls/update_sm_latest.py --start-year 2024
    python scripts/bls/update_sm_latest.py --limit 100
    python scripts/bls/update_sm_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    ...
    parser.add_argument(
        '--supersectors',
        help='Supersector codes (comma-separated)'
    )
    parser.add_argument(
        '--industries',
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW SM DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
    ...
    try:
        ...
        if args.series_ids:
            ...
            if args.states:
                ...
                query = query.filter(SMSeries.state_code.in_(filter_values))
                print(f"Filter: state_code in {filter_values}")
            if args.areas:
                filter_values = [v.strip() for v in args.areas.split(',')]
                query = query.filter(SMSeries.area_code.in_(filter_values))
                print(f"Filter: area_code in {filter_values}")
            if args.supersectors:
                filter_values = [v.strip() for v in args.supersectors.split(',')]
                query = query.filter(SMSeries.supersector_code.in_(filter_values))
                print(f"Filter: supersector_code in {filter_values}")
            if args.industries:
                filter_values = [v.strip() for v in args.industries.split(',')]
                query = query.filter(SMSeries.industry_code.in_(filter_values))
                print(f"Filter: industry_code in {filter_values}")
            if args.data_types:
                filter_values = [v.strip() for v in args.data_types.split(',')]
                query = query.filter(SMSeries.data_type_code.in_(filter_values))
                print(f"Filter: data_type_code in {filter_values}")
            if args.seasonal:
                filter_values = [v.strip() for v in args.seasonal.split(',')]
                query = query.filter(SMSeries.seasonal_code.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'sm',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                SMData.series_id,
                func.max(SMData.year).label('max_year')
            ).filter(
                SMData.series_id.in_(series_ids)
            ).group_by(
                SMData.series_id
            ).all()
