#!/usr/bin/env python3
"""
Update LN data with latest observations from BLS API

This script fetches the latest data points for LN series via the BLS API
...
Usage:
    ...
    python scripts/bls/update_ln_latest.py --limit 100
    python scripts/bls/update_ln_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    ...
    parser.add_argument(
        '--labor-force-status',
        help='Labor force status codes (comma-separated)'
    )
    parser.add_argument(
        '--ages',
        help='Age group codes (comma-separated)'
    )
    parser.add_argument(
        '--sex',
        help='Sex codes (comma-separated)'
    )
    parser.add_argument(
        '--race',
        help='Race codes (comma-separated)'
    )
    parser.add_argument(
        '--education',
        help='Education codes (comma-separated)'
    )
    parser.add_argument(
        '--occupations',
        help='Occupation codes (comma-separated)'
    )
    parser.add_argument(
        '--industries',
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW LN DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING LN (Labor Force Statistics from the Current Population Survey) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if args.series_ids:
            ...
            if args.labor_force_status:
                filter_values = [v.strip() for v in args.labor_force_status.split(',')]
                query = query.filter(LNSeries.lfst_code.in_(filter_values))
                print(f"Filter: lfst_code in {filter_values}")
            if args.ages:
                filter_values = [v.strip() for v in args.ages.split(',')]
                query = query.filter(LNSeries.ages_code.in_(filter_values))
                print(f"Filter: ages_code in {filter_values}")
            if args.sex:
                filter_values = [v.strip() for v in args.sex.split(',')]
                query = query.filter(LNSeries.sexs_code.in_(filter_values))
                print(f"Filter: sexs_code in {filter_values}")
            if args.race:
                filter_values = [v.strip() for v in args.race.split(',')]
                query = query.filter(LNSeries.race_code.in_(filter_values))
                print(f"Filter: race_code in {filter_values}")
            if args.education:
                filter_values = [v.strip() for v in args.education.split(',')]
                query = query.filter(LNSeries.education_code.in_(filter_values))
                print(f"Filter: education_code in {filter_values}")
            if args.occupations:
                filter_values = [v.strip() for v in args.occupations.split(',')]
                query = query.filter(LNSeries.occupation_code.in_(filter_values))
                print(f"Filter: occupation_code in {filter_values}")
            if args.industries:
                filter_values = [v.strip() for v in args.industries.split(',')]
                query = query.filter(LNSeries.indy_code.in_(filter_values))
                print(f"Filter: indy_code in {filter_values}")
            if args.seasonal:
                filter_values = [v.strip() for v in args.seasonal.split(',')]
                query = query.filter(LNSeries.seasonal.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'ln',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                LNData.series_id,
                func.max(LNData.year).label('max_year')
            ).filter(
                LNData.series_id.in_(series_ids)
            ).group_by(
                LNData.series_id
            ).all()
