#!/usr/bin/env python3
"""
Update JT data with latest observations from BLS API

This script fetches the latest data points for JT series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_jt_latest.py
    python scripts/bls/update_jt_latest.py --start-year 2024
    python scripts/bls/update_jt_latest.py --limit 100
    python scripts/bls/update_jt_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW JT DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING JT (JOLTS (Job Openings and Labor Turnover Survey)) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'jt',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                JTData.series_id,
                func.max(JTData.year).label('max_year')
            ).filter(
                JTData.series_id.in_(series_ids)
            ).group_by(
                JTData.series_id
            ).all()
