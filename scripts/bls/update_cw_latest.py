#!/usr/bin/env python3
"""
Update CW data with latest observations from BLS API
...
Usage:
    ...
    python scripts/bls/update_cw_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW CW DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING CW (Consumer Price Index for Urban Wage Earners) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'cw',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                CWData.series_id,
                func.max(CWData.year).label('max_year')
            ).filter(
                CWData.series_id.in_(series_ids)
            ).group_by(
                CWData.series_id
            ).all()
