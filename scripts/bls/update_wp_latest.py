#!/usr/bin/env python3
"""
Update WP data with latest observations from BLS API
...
Usage:
    ...
    python scripts/bls/update_wp_latest.py --start-year 2024
    python scripts/bls/update_wp_latest.py --limit 100
    python scripts/bls/update_wp_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Update WP data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW WP DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING WP (Producer Price Index) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if args.series_ids:
            ...
            if args.groups:
                ...
                query = query.filter(WPSeries.group_code.in_(filter_values))
                print(f"Filter: group_code in {filter_values}")
            if args.items:
                filter_values = [v.strip() for v in args.items.split(',')]
                query = query.filter(WPSeries.item_code.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'wp',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                WPData.series_id,
                func.max(WPData.year).label('max_year')
            ).filter(
                WPData.series_id.in_(series_ids)
            ).group_by(
                WPData.series_id
            ).all()
