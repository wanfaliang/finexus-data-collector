#!/usr/bin/env python3
"""
Update AP data with latest observations from BLS API

This script fetches the latest data points for AP series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_ap_latest.py
    python scripts/bls/update_ap_latest.py --start-year 2024
    python scripts/bls/update_ap_latest.py --limit 100
    python scripts/bls/update_ap_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
from database.bls_models import APSeries, APData
from database.bls_tracking_models import BLSSeriesUpdateStatus, BLSAPIUsageLog
from config import settings

def main():
    parser = argparse.ArgumentParser(description="Update AP data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW AP DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING AP (Average Price Data) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if args.series_ids:
            ...
            query = session.query(APSeries.series_id).filter(APSeries.is_active == True)
            # Apply survey-specific filters
            if args.areas:
                filter_values = [v.strip() for v in args.areas.split(',')]
                query = query.filter(APSeries.area_code.in_(filter_values))
                print(f"Filter: area_code in {filter_values}")
            if args.items:
                filter_values = [v.strip() for v in args.items.split(',')]
                query = query.filter(APSeries.item_code.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'ap',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                APData.series_id,
                func.max(APData.year).label('max_year')
            ).filter(
                APData.series_id.in_(series_ids)
            ).group_by(
                APData.series_id
            ).all()
            ...
            for batch_num, i in enumerate(range(0, len(series_ids), batch_size), 1):
                ...
                try:
                    ...
                    if data_to_upsert:
                        stmt = insert(APData).values(data_to_upsert)
