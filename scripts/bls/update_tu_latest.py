#!/usr/bin/env python3
"""
Update TU data with latest observations from BLS API

This script fetches the latest data points for TU series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_tu_latest.py
    python scripts/bls/update_tu_latest.py --start-year 2024
    python scripts/bls/update_tu_latest.py --limit 100
    python scripts/bls/update_tu_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    ...
    parser.add_argument(
        '--stat-types',
        help='Statistic type codes (comma-separated)'
    )
    parser.add_argument(
        '--sex',
    ...
    parser.add_argument(
        '--regions',
        help='Region codes (comma-separated)'
    )
    parser.add_argument(
        '--labor-force-status',
    ...
    parser.add_argument(
        '--activities',
        help='Activity codes (comma-separated)'
    )
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW TU DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
    ...
    try:
        ...
        if args.series_ids:
            ...
            if args.stat_types:
                filter_values = [v.strip() for v in args.stat_types.split(',')]
                query = query.filter(TUSeries.stattype_code.in_(filter_values))
                print(f"Filter: stattype_code in {filter_values}")
            if args.sex:
                filter_values = [v.strip() for v in args.sex.split(',')]
                query = query.filter(TUSeries.sex_code.in_(filter_values))
                print(f"Filter: sex_code in {filter_values}")
            if args.regions:
                filter_values = [v.strip() for v in args.regions.split(',')]
                query = query.filter(TUSeries.region_code.in_(filter_values))
                print(f"Filter: region_code in {filter_values}")
            if args.labor_force_status:
                filter_values = [v.strip() for v in args.labor_force_status.split(',')]
                query = query.filter(TUSeries.lfstat_code.in_(filter_values))
                print(f"Filter: lfstat_code in {filter_values}")
            if args.activities:
                filter_values = [v.strip() for v in args.activities.split(',')]
                query = query.filter(TUSeries.actcode_code.in_(filter_values))
                print(f"Filter: actcode_code in {filter_values}")
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'tu',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                TUData.series_id,
                func.max(TUData.year).label('max_year')
            ).filter(
