#!/usr/bin/env python3
"""
Update CU data with latest observations from BLS API

This script fetches the latest data points for CU series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_cu_latest.py
    python scripts/bls/update_cu_latest.py --start-year 2024
    python scripts/bls/update_cu_latest.py --limit 100
    python scripts/bls/update_cu_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
from database.bls_models import CUSeries, CUData
from database.bls_tracking_models import BLSSeriesUpdateStatus, BLSAPIUsageLog
from config import settings

def main():
    parser = argparse.ArgumentParser(description="Update CU data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW CU DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING CU (Consumer Price Index (All Urban Consumers)) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'cu',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                CUData.series_id,
                func.max(CUData.year).label('max_year')
            ).filter(
                CUData.series_id.in_(series_ids)
            ).group_by(
                CUData.series_id
            ).all()
            ...
            for batch_num, i in enumerate(range(0, len(series_ids), batch_size), 1):
                ...
                try:
                    ...
                    if data_to_upsert:
                        stmt = insert(CUData).values(data_to_upsert)
