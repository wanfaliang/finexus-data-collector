#!/usr/bin/env python3
"""
Update EI data with latest observations from BLS API

This script fetches the latest data points for EI series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_ei_latest.py
    python scripts/bls/update_ei_latest.py --start-year 2024
    python scripts/bls/update_ei_latest.py --limit 100
    python scripts/bls/update_ei_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
from database.bls_models import EISeries, EIData
from database.bls_tracking_models import BLSSeriesUpdateStatus, BLSAPIUsageLog
from config import settings

def main():
    parser = argparse.ArgumentParser(description="Update EI data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    parser.add_argument(
        '--indexes',
        help='Index codes (comma-separated)'
    )
    parser.add_argument(
        '--seasonal',
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW EI DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING EI (Import/Export Price Indexes) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if args.series_ids:
            ...
            query = session.query(EISeries.series_id).filter(EISeries.is_active == True)
            # Apply survey-specific filters
            if args.indexes:
                filter_values = [v.strip() for v in args.indexes.split(',')]
                query = query.filter(EISeries.index_code.in_(filter_values))
                print(f"Filter: index_code in {filter_values}")
            if args.seasonal:
                filter_values = [v.strip() for v in args.seasonal.split(',')]
                query = query.filter(EISeries.seasonal_code.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'ei',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                EIData.series_id,
                func.max(EIData.year).label('max_year')
            ).filter(
                EIData.series_id.in_(series_ids)
            ).group_by(
                EIData.series_id
            ).all()
                ...
                try:
                    ...
                    for series_id in batch:
                        ...
                        latest = session.query(
                            func.max(EIData.year)
                        ).filter(
                            EIData.series_id == series_id
                        ).scalar()
