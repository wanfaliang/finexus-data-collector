#!/usr/bin/env python3
"""
Update BD data with latest observations from BLS API

This script fetches the latest data points for BD series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_bd_latest.py
    python scripts/bls/update_bd_latest.py --start-year 2024
    python scripts/bls/update_bd_latest.py --limit 100
    python scripts/bls/update_bd_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
from database.bls_models import BDSeries, BDData
from database.bls_tracking_models import BLSSeriesUpdateStatus, BLSAPIUsageLog
from config import settings

def main():
    parser = argparse.ArgumentParser(description="Update BD data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    parser.add_argument(
        '--states',
        help='State codes (comma-separated)'
    )
    parser.add_argument(
        '--industries',
    ...
    parser.add_argument(
        '--unit-analysis',
        help='Unit of analysis codes (comma-separated)'
    )
    parser.add_argument(
        '--data-elements',
        help='Data element codes (comma-separated)'
    )
    parser.add_argument(
        '--size-classes',
        help='Size class codes (comma-separated)'
    )
    parser.add_argument(
        '--data-classes',
        help='Data class codes (comma-separated)'
    )
    parser.add_argument(
        '--rate-levels',
        help='Rate/level codes (comma-separated)'
    )
    parser.add_argument(
        '--seasonal',
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW BD DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
        print("UPDATING BD (Business Employment Dynamics) DATA FROM BLS API")
    print("=" * 80)
    ...
    try:
        ...
        if args.series_ids:
            ...
            query = session.query(BDSeries.series_id).filter(BDSeries.is_active == True)
            # Apply survey-specific filters
            if args.states:
                filter_values = [v.strip() for v in args.states.split(',')]
                query = query.filter(BDSeries.state_code.in_(filter_values))
                print(f"Filter: state_code in {filter_values}")
            if args.industries:
                filter_values = [v.strip() for v in args.industries.split(',')]
                query = query.filter(BDSeries.industry_code.in_(filter_values))
                print(f"Filter: industry_code in {filter_values}")
            if args.unit_analysis:
                filter_values = [v.strip() for v in args.unit_analysis.split(',')]
                query = query.filter(BDSeries.unitanalysis_code.in_(filter_values))
                print(f"Filter: unitanalysis_code in {filter_values}")
            if args.data_elements:
                filter_values = [v.strip() for v in args.data_elements.split(',')]
                query = query.filter(BDSeries.dataelement_code.in_(filter_values))
                print(f"Filter: dataelement_code in {filter_values}")
            if args.size_classes:
                filter_values = [v.strip() for v in args.size_classes.split(',')]
                query = query.filter(BDSeries.sizeclass_code.in_(filter_values))
                print(f"Filter: sizeclass_code in {filter_values}")
            if args.data_classes:
                filter_values = [v.strip() for v in args.data_classes.split(',')]
                query = query.filter(BDSeries.dataclass_code.in_(filter_values))
                print(f"Filter: dataclass_code in {filter_values}")
            if args.rate_levels:
                filter_values = [v.strip() for v in args.rate_levels.split(',')]
                query = query.filter(BDSeries.ratelevel_code.in_(filter_values))
                print(f"Filter: ratelevel_code in {filter_values}")
            if args.seasonal:
                filter_values = [v.strip() for v in args.seasonal.split(',')]
                query = query.filter(BDSeries.seasonal_code.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'bd',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                BDData.series_id,
                func.max(BDData.year).label('max_year')
            ).filter(
            ...
            for batch_num, i in enumerate(range(0, len(series_ids), batch_size), 1):
                ...
                try:
                    ...
                    if data_to_upsert:
                        stmt = insert(BDData).values(data_to_upsert)
                    ...
                    for series_id in batch:
                        ...
                        latest = session.query(
                            func.max(BDData.year)
                        ).filter(
                            BDData.series_id == series_id
                        ).scalar()
