#!/usr/bin/env python3
"""
Update OE data with latest observations from BLS API

This script fetches the latest data points for OE series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_oe_latest.py
    python scripts/bls/update_oe_latest.py --start-year 2024
    python scripts/bls/update_oe_latest.py --limit 100
    python scripts/bls/update_oe_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Update OE data with latest from BLS API")
    parser.add_argument(
        '--start-year',
        type=int,
    ...
    parser.add_argument(
        '--area-types',
        help='Area type codes (comma-separated)'
    )
    parser.add_argument(
        '--industries',
    ...
    parser.add_argument(
        '--data-types',
        help='Data type codes (comma-separated)'
    )
    parser.add_argument(
        '--states',
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW OE DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
    ...
    try:
        ...
        if args.series_ids:
            ...
            query = session.query(OESeries.series_id).filter(OESeries.is_active == True)
            # Apply survey-specific filters
            if args.area_types:
                filter_values = [v.strip() for v in args.area_types.split(',')]
                query = query.filter(OESeries.areatype_code.in_(filter_values))
                print(f"Filter: areatype_code in {filter_values}")
            if args.industries:
                filter_values = [v.strip() for v in args.industries.split(',')]
                query = query.filter(OESeries.industry_code.in_(filter_values))
                print(f"Filter: industry_code in {filter_values}")
            if args.occupations:
                filter_values = [v.strip() for v in args.occupations.split(',')]
                query = query.filter(OESeries.occupation_code.in_(filter_values))
                print(f"Filter: occupation_code in {filter_values}")
            if args.data_types:
                filter_values = [v.strip() for v in args.data_types.split(',')]
                query = query.filter(OESeries.datatype_code.in_(filter_values))
                print(f"Filter: datatype_code in {filter_values}")
            if args.states:
                filter_values = [v.strip() for v in args.states.split(',')]
                query = query.filter(OESeries.state_code.in_(filter_values))
                print(f"Filter: state_code in {filter_values}")
            if args.areas:
                filter_values = [v.strip() for v in args.areas.split(',')]
                query = query.filter(OESeries.area_code.in_(filter_values))
                print(f"Filter: area_code in {filter_values}")
            if args.sectors:
                filter_values = [v.strip() for v in args.sectors.split(',')]
                query = query.filter(OESeries.sector_code.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'oe',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                OEData.series_id,
                func.max(OEData.year).label('max_year')
            ).filter(
            ...
            max_periods_per_series = years_to_fetch * 1
            ...
            print(f"  Max periods per series: {max_periods_per_series} (annual)")
                ...
                try:
                    ...
                    for series_id in batch:
                        ...
                        latest = session.query(
                            func.max(OEData.year)
                        ).filter(
                            OEData.series_id == series_id
                        ).scalar()
