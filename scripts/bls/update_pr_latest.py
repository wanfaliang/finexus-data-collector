#!/usr/bin/env python3
"""
Update PR data with latest observations from BLS API

This script fetches the latest data points for PR series via the BLS API
and updates the database. Use this for regular updates after initial load.

Usage:
    python scripts/bls/update_pr_latest.py
    python scripts/bls/update_pr_latest.py --start-year 2024
    python scripts/bls/update_pr_latest.py --limit 100
    python scripts/bls/update_pr_latest.py --dry-run  # Preview without fetching
"""
import sys
import argparse
...
def main():
    ...
    parser.add_argument(
        '--classes',
        help='Class codes (comma-separated)'
    )
    parser.add_argument(
        '--measures',
    ...
    print("=" * 80)
    if args.dry_run:
        print("DRY RUN: PREVIEW PR DATA UPDATE (NO CHANGES WILL BE MADE)")
    else:
    ...
    try:
        ...
        if args.series_ids:
            ...
            if args.sectors:
                ...
                query = query.filter(PRSeries.sector_code.in_(filter_values))
                print(f"Filter: sector_code in {filter_values}")
            if args.classes:
                filter_values = [v.strip() for v in args.classes.split(',')]
                query = query.filter(PRSeries.class_code.in_(filter_values))
                print(f"Filter: class_code in {filter_values}")
            if args.measures:
                filter_values = [v.strip() for v in args.measures.split(',')]
                query = query.filter(PRSeries.measure_code.in_(filter_values))
                print(f"Filter: measure_code in {filter_values}")
            if args.durations:
                filter_values = [v.strip() for v in args.durations.split(',')]
                query = query.filter(PRSeries.duration_code.in_(filter_values))
                print(f"Filter: duration_code in {filter_values}")
            if args.seasonal:
                filter_values = [v.strip() for v in args.seasonal.split(',')]
                query = query.filter(PRSeries.seasonal.in_(filter_values))
        ...
        if not args.series_ids and not args.force:  # Only auto-filter if not explicitly specified or forced
            ...
            current_series = session.query(
                ...
                BLSSeriesUpdateStatus.survey_code == 'pr',
        ...
        if args.dry_run:
            ...
            latest_data = session.query(
                PRData.series_id,
                func.max(PRData.year).label('max_year')
            ).filter(
                PRData.series_id.in_(series_ids)
            ).group_by(
                PRData.series_id
            ).all()
                ...
                try:
                    ...
                    for series_id in batch:
                        ...
                        latest = session.query(
                            func.max(PRData.year)
                        ).filter(
                            PRData.series_id == series_id
                        ).scalar()
